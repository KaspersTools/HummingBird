  ...
  namespace ThirdParty {
    namespace Intel {
      ...
      MetricsGuiPlot::MetricsGuiPlot()
          : mMetrics(), mMetricRange(), mWidthInfo(new MetricsGuiPlot::WidthInfo(this)), mMinValue(0.f), mMaxValue(0.f),
            mRangeInitialized(false), mBarRounding(0.f), mRangeDampening(0.95f), mInlinePlotRowCount(2), mPlotRowCount(5),
            mVBarMinWidth(6), mVBarGapWidth(1), mShowAverage(false), mShowInlineGraphs(false), mShowOnlyIfSelected(false),
            mShowLegendDesc(true), mShowLegendColor(true), mShowLegendUnits(true), mShowLegendAverage(false),
            mShowLegendMin(true), mShowLegendMax(true), mBarGraph(false), mStacked(false), mSharedAxis(false),
            mFilterHistory(true) {
      }

      MetricsGuiPlot::MetricsGuiPlot(
              MetricsGuiPlot const &copy)
          : mMetrics(copy.mMetrics), mMetricRange(copy.mMetricRange), mWidthInfo(copy.mWidthInfo),
            mMinValue(copy.mMinValue), mMaxValue(copy.mMaxValue), mRangeInitialized(copy.mRangeInitialized),
            mBarRounding(copy.mBarRounding), mRangeDampening(copy.mRangeDampening),
            mInlinePlotRowCount(copy.mInlinePlotRowCount), mPlotRowCount(copy.mPlotRowCount),
            mVBarMinWidth(copy.mVBarMinWidth), mVBarGapWidth(copy.mVBarGapWidth), mShowAverage(copy.mShowAverage),
            mShowInlineGraphs(copy.mShowInlineGraphs), mShowOnlyIfSelected(copy.mShowOnlyIfSelected),
            mShowLegendDesc(copy.mShowLegendDesc), mShowLegendColor(copy.mShowLegendColor),
            mShowLegendUnits(copy.mShowLegendUnits), mShowLegendAverage(copy.mShowLegendAverage),
      ...
      namespace {
        ...
        void DrawMetrics(
          ...
          ImRect frame_bb(
                  window->DC.CursorPos,
                  cursorPos + ImVec2(plotWidth, plotHeight));
          ImRect inner_bb(
          ...
          if (valueX >= barStartX) {
            ...
            if (barEndX > barStartX) {
              auto normalizedValue = metricRange.second > metricRange.first
                                             ? ImSaturate(
                                                       (lastValue - metricRange.first) / (metricRange.second - metricRange.first))
